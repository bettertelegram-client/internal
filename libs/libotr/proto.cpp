	...
	DH_sesskeys *sess1, DH_sesskeys *sess2)
{
    ...
    newmacs = (unsigned char*)realloc(context->context_priv->saved_mac_keys,
	    newnumsaved * 20);
    if (!newmacs) {
...
char *otrl_proto_default_query_msg(const char *ourname, OtrlPolicy policy)
{
     ...
     * free()ing). */
    const char *format = "?OTR%s\n%s has requested to secure this chat with BetterTelegram's OTR (Off-The-Record) feature. For more information, visit https://bettertelegram.org";
    ...
    *bufp = '\0';
    ...
    /* Remove two "%s", add '\0' */
    msg = (char*) malloc(strlen(format) + strlen(version_tag) + strlen(ourname) - 3 + 1);
    if (!msg) {
	free(version_tag);
	return NULL;
    }
    int len = sprintf(msg, format, version_tag, ourname);
    *(msg + len) = '\0';
    free(version_tag);
    return msg;
}
	...
	OtrlPolicy policy)
{
    ...
    otrtag = (char*)strstr(otrquerymsg, "?OTR");
    if (!otrtag) {
	return 0;
    }
    otrtag += 4;
...
OtrlMessageType otrl_proto_message_type(const char *message)
{
    ...
    otrtag = (char*)strstr(message, "?OTR");
	...
	unsigned int *instance_from, unsigned int *instance_to)
{
    ...
    bufp = (unsigned char*)malloc(OTRL_B64_MAX_DECODED_SIZE(12));
    bufp_head = bufp;
	...
	unsigned char *extrakey)
{
    ...
    *encmessagep = NULL;
    ...
    msgbuf = (unsigned char*)gcry_malloc_secure(msglen);
    ...
    *encmessagep = base64buf;
    ...
    if (msglen > 0) {
	context->context_priv->lastmessage = (char*)gcry_malloc_secure(justmsglen + 1);
	...
	unsigned char *flagsp)
{
    ...
    if (flagsp) *flagsp = 0;
    otrtag = (char*)strstr(datamsg, "?OTR:");
    if (!otrtag) {
	goto invval;
    }
	...
	ConnContext *context, const char *msg)
{
	...
	if (k == 1) {
		...
		context->context_priv->fragment = (char*)malloc(newsize);
	    }
		...
		newfrag = (char*)realloc(context->context_priv->fragment, newsize);
	    }
	    if (newfrag) {
	...
	char ***fragments, ConnContext *context, const char *message)
{
    ...
    fragmentarray = (char**) malloc(fragment_count * sizeof(char*));
	...
	fragdata = (char*)malloc(fragdatalen + 1);
	if(!fragdata) {
	...
	fragmentmsg = (char*)malloc(fragdatalen+headerlen+1);
	if(!fragmentmsg) {
	    for (i=0; i<curfrag-1; free(fragmentarray[i++])) {}
	    free(fragmentarray);
	    free(fragdata);
