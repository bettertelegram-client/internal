...
{
    gcry_mpi_t *msg = (gcry_mpi_t*) malloc(SM_MSG1_LEN * sizeof(gcry_mpi_t));
...
{
    gcry_mpi_t *msg = (gcry_mpi_t*) malloc(SM_MSG2_LEN * sizeof(gcry_mpi_t));
    msg[0] = gcry_mpi_new(SM_MOD_LEN_BITS);
    msg[1] = NULL;
    msg[2] = gcry_mpi_new(SM_MOD_LEN_BITS);
    msg[3] = gcry_mpi_new(SM_MOD_LEN_BITS);
...
{
    gcry_mpi_t *msg = (gcry_mpi_t *) malloc(SM_MSG3_LEN * sizeof(gcry_mpi_t));
    msg[0] = gcry_mpi_new(SM_MOD_LEN_BITS);
...
{
    gcry_mpi_t *msg = (gcry_mpi_t *)malloc(SM_MSG4_LEN * sizeof(gcry_mpi_t));
    msg[0] = gcry_mpi_new(SM_MOD_LEN_BITS);
    msg[1] = NULL;
    msg[2] = gcry_mpi_new(SM_MOD_LEN_BITS);
...
static gcry_mpi_t randomExponent(void)
{
    ...
    secbuf = (unsigned char*)gcry_random_bytes_secure(SM_MOD_LEN_BYTES, GCRY_STRONG_RANDOM);
	...
	const gcry_mpi_t a, const gcry_mpi_t b)
{
    ...
    input = (unsigned char*)malloc(totalsize);
	...
	unsigned int count, gcry_mpi_t *mpis)
{
    ...
    unsigned int i, j;
    size_t *list_sizes = (size_t*) malloc(count * sizeof(size_t));
    unsigned char **tempbuffer = (unsigned char**) malloc(count * sizeof(unsigned char *));
    unsigned char *bufp;
    ...
    *buffer = (unsigned char*)malloc(*buflen * sizeof(char));
	...
	unsigned int expcount, const unsigned char *buffer, const int buflen)
{
    ...
    *mpis = (gcry_mpi_t*)malloc(thecount * sizeof(gcry_mpi_t));
