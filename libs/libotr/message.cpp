	...
	OtrlFragmentPolicy fragPolicy, char **returnFragment)
{
	...
	if(mms != 0 && msglen > mms) {
		...
		*returnFragment = _strdup(message);
	    }
	}
    }
	...
	void *data)
{
	...
	*contextp = context;
	...
	context->otr_offer =ConnContext::OFFER_SENT;
	err = gcry_error(GPG_ERR_NO_ERROR);
	goto fragment;
    }
	...
	case OTRL_MSGSTATE_PLAINTEXT:
		...
		context->context_priv->lastmessage = (char*)
		...
		if ((policy & OTRL_POLICY_SEND_WHITESPACE_TAG) &&
			context->otr_offer !=ConnContext::OFFER_REJECTED) {
	...
	case OTRL_MSGSTATE_ENCRYPTED:
		...
		*messagep = msgtosend;
		...
		if (ops->otr_error_message) {
		    ...
		    *messagep = (char*) malloc(strlen(OTR_ERROR_PREFIX) +
			strlen(err_msg) + 1);
		    if (*messagep) {
	...
	const unsigned char *secret, size_t secretlen, int initiating)
{
    ...
    combined_buf = (unsigned char*)malloc(combined_buf_len);
    combined_buf[0] = 0x01;
    if (initiating) {
	...
	unsigned char *qsmpmsg = (unsigned char*) malloc(qlen + 1 + smpmsglen);
	if (!qsmpmsg) {
	    free(smpmsg);
	    return;
	}
...
static void message_malformed(const OtrlMessageAppOps *ops,
	...
	if (err_msg) {
	    char *buf = (char*)malloc(strlen(OTR_ERROR_PREFIX) + strlen(err_msg) + 1);

	    if (buf) {
		strcpy(buf, OTR_ERROR_PREFIX);
		strcat(buf, err_msg);
		*(buf + strlen(OTR_ERROR_PREFIX) + strlen(err_msg)) = '\0';
		ops->inject_message(opdata, context->accountname,
	...
	void *data)
{
	...
	*contextp = NULL;
	...
	if (msgtype != OTRL_MSGTYPE_NOTOTR) {
	    context->otr_offer =ConnContext::OFFER_ACCEPTED;
	} else if (context->otr_offer ==ConnContext::OFFER_SENT) {
	    context->otr_offer =ConnContext::OFFER_REJECTED;
	}
    }
	...
	*contextp = context;
	...
	case OTRL_MSGTYPE_DATA:
		...
		case OTRL_MSGSTATE_ENCRYPTED:
		    extrakey = (unsigned char*)gcry_malloc_secure(OTRL_EXTRAKEY_BYTES);
			...
			if (ops->inject_message && ops->otr_error_message) {
			    err_msg = ops->otr_error_message(opdata,
					...
					is_conflict ?
					    ...
					    OTRL_ERRCODE_MSG_MALFORMED);
			    if (err_msg) {
				buf = (char*)malloc(strlen(OTR_ERROR_PREFIX) +
						strlen(err_msg) + 1);
				if (buf) {
				    strcpy(buf, OTR_ERROR_PREFIX);
				    strcat(buf, err_msg);
					*(buf + strlen(OTR_ERROR_PREFIX) +
						strlen(err_msg)) = '\0';
			...
			if (nextMsg == OTRL_SMP_EXPECT1 && tlv->len > 0) {
			    ...
			    char *qend = (char*)memchr(question, '\0', tlv->len - 1);
			...
			*newmessagep = plaintext;
			...
			if (ops->convert_msg) {
				...
				free(plaintext);
				plaintext = NULL;
				*newmessagep = _strdup(converted_msg);
	...
	unsigned char *symkey)
{
    ...
    if (context->msgstate == OTRL_MSGSTATE_ENCRYPTED &&
	    context->context_priv->their_keyid > 0) {
	unsigned char *tlvdata = (unsigned char*) malloc(usedatalen+4);
	char *encmsg = NULL;
	gcry_error_t err;
	OtrlTLV *tlv;
