	...
	const char *accountname, const char *protocol)
{
    ...
    search = (OtrlPendingPrivKey*) malloc(sizeof(*search));
    if (!search) return NULL;

    search->accountname = _strdup(accountname);
    search->protocol = _strdup(protocol);
	...
	const char *accountname, const char *protocol, void **newkeyp)
{
    ...
    ppc = (s_pending_privkey_calc*)malloc(sizeof(*ppc));
    ppc->accountname = _strdup(accountname);
    ppc->protocol = _strdup(protocol);
    ppc->privkey = NULL;
