...
gcry_error_t otrl_instag_read_FILEp(OtrlUserState us, FILE *instf)
{
	...
	*pos = '\0';
	pos++;
	p->accountname = (char*)malloc(pos - prevpos);
	...
	*pos = '\0';
	pos++;
	p->protocol = (char*)malloc(pos - prevpos);
	...
	const char *accountname, const char *protocol)
{
    ...
    p->accountname = _strdup(accountname);
    p->protocol = _strdup(protocol);
